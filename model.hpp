...
static unsigned int compileShader(GLenum type, const char* source) {
    ...
    if (!file.is_open()) {
        std::cout << "ERROR: Shader file not found: " << source << std::endl;
        return 0;
    }
...
// Create simple procedural textures if files don't exist
unsigned int createSandTexture() {
    const int size = 256;
    unsigned char* data = new unsigned char[size * size * 3];

    for (int i = 0; i < size * size; i++) {
        data[i * 3 + 0] = 194 + (rand() % 20) - 10; // R
        data[i * 3 + 1] = 178 + (rand() % 20) - 10; // G
        data[i * 3 + 2] = 128 + (rand() % 20) - 10; // B
    }
...
unsigned int createWoodTexture() {
    ...
    for (int y = 0; y < size; y++) {
        for (int x = 0; x < size; x++) {
            ...
            data[i + 0] = 139 + variation; // R
            data[i + 1] = 90 + variation;  // G
            data[i + 2] = 43 + variation;  // B
        }
    }
